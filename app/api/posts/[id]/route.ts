...
export async function PUT(request: NextRequest, { params }: RouteContext) {
  try {
    ...
    // Allow both admin and editor roles to update posts
    if (!['admin', 'editor'].includes(user.role)) {
      return NextResponse.json(
        { error: `Access denied. Your role "${user.role}" cannot update posts. Admin or editor role required.` },
        { status: 403 }
      )
    }
...
export async function DELETE(request: NextRequest, { params }: RouteContext) {
  try {
    ...
    if (user.role !== 'admin') {
      return NextResponse.json(
        { error: `Access denied. Only admins can delete posts. Your role is "${user.role}".` },
        { status: 403 }
      )
    }
