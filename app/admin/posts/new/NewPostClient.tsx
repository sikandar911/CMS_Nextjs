...
export default function NewPostClient({ categories }: NewPostClientProps) {
  ...
  const handlePostChange = (field: keyof Post, value: string | string[] | number | undefined) => {
    setPost(prev => {
  ...
  const handleSave = async (publishStatus: 'draft' | 'published' = 'draft') => {
    ...
    try {
      ...
      const response = await clientApi.posts.create(postData as any)
  ...
  const handleCancel = async () => {
    if (hasUnsavedChanges) {
      const confirmed = await confirm('You have unsaved changes. Are you sure you want to leave without saving?')
      if (!confirmed) return
    }
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) => handlePostChange('category_id', e.target.value ? parseInt(e.target.value) : undefined)}
                      className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 text-gray-900 bg-white"
                    >
              ...
              <div className="space-y-6">
                <div className="bg-gray-50 p-6 rounded-lg">
                  ...
                  <div className="text-sm text-gray-500">
                    Category: {categories.find(c => c.id === post.category_id)?.name || 'None'} | Status: {post.status} | Slug: {post.slug}
                  </div>
                </div>
